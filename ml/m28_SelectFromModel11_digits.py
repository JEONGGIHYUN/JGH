...
x_train, x_test, y_train, y_test = train_test_split(x, y, train_size=0.8, random_state=213,shuffle=True,stratify=y)
...
early_stop = xgb.callback.EarlyStopping(
    rounds=50,
    metric_name='mlogloss', #이진은 logloss, 다중은 mlogloss
...
model = XGBClassifier(
    ...
    random_state=2146,
    eval_metric='mlogloss', # 2.1.1 버전에서 여기서 써야 한다.
)
...
for i in thresholds:
    ...
    select_model = XGBClassifier(  n_estimators = 10,
                                    max_depth = 8,
                                    gamma = 0,
                                    min_child_weight= 0,
                                    subsample= 0.4,
