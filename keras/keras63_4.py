...
from tensorflow.keras.layers import Dense, Bidirectional,Concatenate,BatchNormalization, Reshape, GRU, Conv1D, Flatten, Dropout, LSTM
...
import pandas as pd
path = 'C:/TDS/ai5/_data/성우하이텍/'

naver_csv = pd.read_csv(path + "NAVER 240816(2).csv", index_col=0, thousands=',')
naver_csv.columns = ['시가','고가','저가','종가','전일비','전일비2','등락률','거래량','금액(백만)','신용비','개인','기관','외인(수량)','외국계','프로그램','외인비']
hitech_csv = pd.read_csv(path + "성우하이텍 240816(1).csv", index_col=0, thousands=',')
hitech_csv.columns = ['시가','고가','저가','종가','전일비','전일비2','등락률','거래량','금액(백만)','신용비','개인','기관','외인(수량)','외국계','프로그램','외인비']
hybe_csv = pd.read_csv(path + "하이브 240816(1).csv", index_col=0, thousands=',')
...
reshape2 = Reshape(target_shape=(16,64,1))(dense1)
...
reshape2 = Reshape(target_shape=(16,64,1))(dense11)
dense12 = Conv1D(64, kernel_size=2, padding="causal")(reshape2)
dropout3 = Dropout(0.2)(dense12)
