...
np.save(np_path2 + 'x_train2.npy', arr=xy_train[0][0])

np.save(np_path2 + 'y_train2.npy', arr=xy_train[0][1])

np.save(np_path2 + 'x_test2.npy', arr=xy_test[0][0])

np.save(np_path2 + 'y_test2.npy', arr=xy_test[0][1])

# submission_csv = pd.read_csv(path_csv + 'sample_submission.csv', index_col=0)

# x_train, x_test, y_train, y_test = train_test_split(xy_train[0][0], xy_train[0][1], train_size=0.8, random_state=3)
...
# #2. 모델 구성
# model = Sequential()
# model.add(Conv2D(30,(2,2), input_shape=(100,100,3),padding='same',strides=2))
...
# submission_csv.to_csv(path_csv + 'submission_0802_0553.csv')
