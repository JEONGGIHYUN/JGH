...
from tensorflow.keras.layers import Dense, Conv2D, Flatten, Conv1D
...
# y_end = to_categorical(x_pre)

# x_end = x_end.reshape()

print(x, x.shape) # (15, 5)

# print(x_pre, x_pre.shape) # (11, 5)
# (xy == padded_np).all() # 결과가 같은지 확인하는 파이썬 코드

x_train, x_test, y_train, y_test = train_test_split(x, y, train_size=0.8, random_state=3)

print(x_train.shape,x_test.shape,y_train.shape,y_test.shape) # (12, 5) (3, 5) (12,) (3,)
...
model = Sequential()
model.add(Conv1D(filters=10, kernel_size=1, input_shape=(5, 1)))
model.add(Flatten())
model.add(Dense(128))
